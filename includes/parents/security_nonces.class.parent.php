...
class security_nonces
{

	var $security_nonces_id;
	var $security_nonces_nonce;
	var $security_nonces_expires;
	var $pdo;
	var $errors = [];
	var $fields = [];
	var $table = "security_nonces";
    ...
    function load_me() {
        $sql        = "SELECT * FROM `security_nonces` WHERE `security_nonces_id` = ? LIMIT 1";
        $stmt       = $this->pdo->prepare($sql);
        $values     = [$this->security_nonces_id];
